    ...
    <div class="post-preview">
        ...
        <p class="post-meta">
            ...
            @foreach ($post->users as $user)
                <a class="link-opacity-75-hover user-link" href="{{ route('blog.author', $user->id) }}">{{$user->name}}</a>{{$user != $post->users->last() ? ',' : ''}}
            @endforeach
            on {{$post->created_at->format('d-m-Y')}}
        </p>
        <p>
            @foreach ($post->categories as $category)
                <a style="text-decoration: none" class="link-opacity-75-hover category-span" href="{{ route('blog.category', $category->id) }}" >{{$category->name}}</a>{{$category != $post->categories->last() ? ',' : ''}}
            @endforeach
        </p>
    </div>
        <!-- Divider-->
